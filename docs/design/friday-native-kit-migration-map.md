...
开发执行窗口必须先读取 `docs/plans/2026-05-14-native-kit-ui-reconstruction-handoff.zh.md`；该文件定义 HTML 还原、字段契约、动画和验收规则。
...
- 先迁移助手消息、Task Bar、审批和输入区，因为它们属于同一个信息系统；执行过程不再单独成组件，合入助手消息的过程态和结果态展开。
- 工作台顶部栏可作为同批可见改进。
...
- 项目页设计候选已进 HTML 画板：项目同步状态元素、项目同步工作台、项目页预览、分支选择器、协作记录、Git 更新树。
- 这些候选不改变当前 `native-kit-catalog-decisions.json` 的 13/0/1 选择计数；需要用户在画板中确认后再进入正式迁移计数。
...
- FRIDAY 输出改为文档流。
- FRIDAY 身份每组输出出现一次。
- 结果态顶部保留用时：简单回答为 `FRIDAY 已思考 {elapsed}`，复杂任务为 `FRIDAY 已完成工作 {elapsed} >`。
- 简单回答不展示过程内容；复杂任务结果态的过程默认收起，从 FRIDAY 名称或小箭头展开。
- 显性过程态和复杂结果展开态中的每个过程步骤都可以点击展开，展开后显示该步骤对应的工具调用过程。
- 过程展开里不再显示单独的“过程”标题。
- 工具调用细节必须弱于上层过程步骤：高度更低、边框更淡、文本更轻，不抢主步骤注意力。
- 正文、工具调用、产物使用不同但相关的样式。
- 工具调用使用低强调 icon + 工具名 + 操作内容。
- 不使用重卡片承载工具调用。
- 执行过程不再作为单独组件定义；复杂任务的过程信息进入助手消息中的“正在工作”或结果展开态。
- 产物行显示文件类型 icon，至少可区分 Markdown、Canvas/白板、普通笔记或文档。
...
- 项目同步状态元素：定义同步页可复用的短状态词和状态密度，不用英文 Git 指标直出。
- 项目同步工作台：定义当前项目、远端就绪、主动作、最近同步记录和同步检查的组合方式；项目工作范围只作为上下文，不作为同步页检查项。
- 项目页预览：把项目同步工作台、设置摘要、冲突入口和活动记录放到同一页验证层级。
- 分支选择器：为用户选择同步目标分支提供原生控件和风险提示。
- 协作记录：可视化多人协作提交，帮助用户理解本地和远端差异来源。
- Git 更新树：以树状关系展示本地、远端和候选分支的更新方向。
...
- 项目页候选同样不解除 `项目状态组` 暂缓；它们先验证同步页的信息架构和动作层级。
- 分支、协作和更新树是同步页未来能力候选，不等于引入完整 Git 历史浏览器。
...
- 2026-05-22 已补充项目页候选画板，但它们是设计探索，不是首批迁移授权。
...
- 确认 `native-kit-catalog-decisions.json` 仍是 13 保留、0 调整、1 暂缓。
- 确认 `项目状态组` 仍为 `hold`。
...
- [ ] `项目状态组` 没有被迁移或重构。
- [ ] 助手、Task Bar、助手过程展开和审批使用一致的信息语法。
