...
export class PromptContextEngine {
	...
	build(input: PromptContextBuildInput): PromptContextBuildResult {
		...
		const lines = [
			...
			'{"type":"response","assistant":"final response for user","intake":{"complexity":"simple|light|complex|unclear","route":"answer|clarify|plan_and_execute","statement":"first-person understanding","requiresPlan":true,"shouldShowProcess":true,"shouldUseVisiblePlan":true},"plan":{"type":"plan_create","visibility":"visible|internal","tasks":[{"id":"short-stable-id","title":"task title","status":"pending|in_progress|completed|skipped|failed|blocked"}]}}',
			"",
			"Rules:",
			"- Simple tasks must return only the response schema; do not include intake or plan_create for simple direct answers.",
			"- Complex first tool or progress responses may include model-authored intake and plan_create before any tool call.",
