...
const memoryCapabilityPath = path.join(projectRoot, "src/platform/capability/MemoryPersistCapability.ts");
...
async function loadCapabilityModules() {
	return {
		...
		memory: await jiti.import(memoryCapabilityPath),
...
test("builtin execution paths are bridged through dedicated capability modules", async () => {
	...
	assert.match(source, /MemoryPersistCapability/);
	...
	assert.match(source, /memoryPersistCapability\.persistSignals\(/);
...
test("capability bridge modules export executable classes", async () => {
	...
	assert.equal(typeof modules.memory.MemoryPersistCapability, "function");
